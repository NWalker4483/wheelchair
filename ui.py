...
pygame.init()
UDP_IP = os.environ.get("BASE_IP")
UDP_PORT = os.environ.get("CONTROL_PORT")
...
for i, (symbol, dir) in enumerate(zip(symbols, dirs)):
    x, y = dir
    Button(app, text=symbol, command=partial(
        publish, control_update_topic, "%d, %d" % (x, y))).grid(row=i//3, column=i % 3)

for i in range(9):
    Button(app, text=str(i + 1), command=partial(publish, goal_update_topic,
           "%d" % (i + 1))).grid(row=i//3, column=((i % 3) + 3))
